...
export const getDashboardMetrics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [sessionsWeek, painUnresolved, msgsUnread, videosUnreviewed, weekPlanned, weekCompleted] = await Promise.all([
    ...
    const toTreat = (painUnresolved.count || 0) + (msgsUnread.count || 0) + (videosUnreviewed.count || 0);
...
export const getSessionDetail = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [sessR, setsR, fbR, painsR, freeActR, mediaR, techVidR, commentsR] = await Promise.all([
      ...
      supabaseAdmin.from("exercise_comments").select("id, exercise_name, author_role, content, created_at").eq("session_id", data.sessionId).eq("author_role", "member").order("created_at", { ascending: true }),
    ]);
    ...
    if (sessR.data.program_id) {
      ...
      if (pg) { programName = pg.name; programStructureJson = pg.structure == null ? null : JSON.stringify(pg.structure); }
    }
