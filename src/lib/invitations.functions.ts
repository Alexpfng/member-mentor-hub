...
const SITE_NAME = "Colosmartraining";
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.send_email && data.email) {
      ...
      const { error: enqErr } = await supabaseAdmin.rpc("enqueue_email", {
        ...
        payload: {
          message_id: messageId,
          idempotency_key: `invitation-${row.id}`,
          to: data.email,
          from: `${SITE_NAME} <noreply@${FROM_DOMAIN}>`,
          sender_domain: SENDER_DOMAIN,
          subject: `Tu es invité·e à rejoindre ${SITE_NAME}`,
          html,
          text,
      ...
      if (enqErr) {
        ...
        throw new Error(`Email non envoyé: ${enqErr.message}`);
      }
      emailSent = true;
    }
