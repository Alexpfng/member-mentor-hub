...
export default function Login() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--cst-dark-green)' }}>
      <div className="cst-screen cst-hatch" style={{ width: 390, minHeight: 780, padding: '20px 24px 28px', position: 'relative', borderRadius: 16, overflow: 'hidden' }}>
        <div style={hatchOverlay} />
        <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', minHeight: 740 }}>
          <div style={{ display: 'flex', justifyContent: 'center', paddingTop: 8 }}>
            <CSTLogo />
          </div>

          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 24, paddingTop: 24 }}>
              ...
              <div className="cst-col" style={{ gap: 14 }}>
                {mode === 'signup' && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div>
                      ...
                      <input className="cst-input" placeholder="Jordan"
                        value={firstName} onChange={e => setFirstName(e.target.value)} required />
                    </div>
                    <div>
                      <span className="cst-label">NOM</span>
                      <input className="cst-input" placeholder="Ferrer"
                ...
                <div>
                  ...
                  <input className="cst-input" type="password" placeholder="••••••••••"
                    value={password} onChange={e => { setPassword(e.target.value); setError(''); }} required minLength={6} />
                </div>
                ...
                {info && (
                  <div style={{ padding: '10px 14px', background: 'rgba(45,90,53,0.15)', border: '1px solid rgba(45,90,53,0.4)', borderRadius: 8, fontSize: 12, color: '#6EAB76' }}>
                    {info}
                  </div>
                )}
                ...
                <button type="button" onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(''); setInfo(''); }}
                  style={{ background: 'none', border: 'none', textAlign: 'center', fontSize: 11, color: 'var(--cst-text-muted)', cursor: 'pointer', fontFamily: 'inherit' }}>
                  {mode === 'login' ? "Pas encore de compte ? S'inscrire" : "Déjà inscrit ? Se connecter"}
                </button>
                {mode === 'login' && (
                  <button type="button" onClick={handleForgot}
                    style={{ background: 'none', border: 'none', textAlign: 'center', fontSize: 11, color: 'var(--cst-text-muted)', cursor: 'pointer', fontFamily: 'inherit', textDecoration: 'underline' }}>
                    Mot de passe oublié ?
                  </button>
                )}
              </div>
