...
type ProgExo = {
  ...
  reps?: string | number;
  charge?: string;
  rpe_target?: string | number;
  recup?: string;
};
type ProgBlock = { letter?: string; type?: string; isSuperset?: boolean; exercises?: ProgExo[] };
type ProgWeek = {
  number?: number;
  days?: Array<{ number?: number; label?: string; blocks?: ProgBlock[] }>;
};
...
export default function CoachSessionDetail() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: u }) => setCoachUserId(u.user?.id ?? null));
  }, []);
  useEffect(() => {
    if (data?.session)
      setCoachNote((data.session as { coach_note?: string | null }).coach_note ?? "");
  }, [data]);
  ...
  const blockForExo = useMemo(() => {
    ...
    if (!data?.program?.structureJson || !data.session.week_number || !data.session.day_number)
      return map;
    try {
      ...
      : freeCategory === "cardio"
        ...
        : freeCategory === "muscu"
          ...
          : freeCategory === "sport"
            ? "⚽"
            : "✨";
  const freeActivities = (data as any).freeActivities as
    | Array<{
        id: string;
        name: string;
    ...
    | undefined;
  const allMedia = (data as any).media as
    | Array<{
        id: string;
        type: "photo" | "video";
        url: string | null;
  ...
  const techniqueVideos =
    ((data as any).techniqueVideos as
      | Array<{
          id: string;
      ...
      <div className="cst-col cst-scroll" style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <CSTSectionNum
              num={1}
              label={isFree ? "SÉANCE LIBRE" : "DÉTAIL DE SÉANCE"}
              sub={`${data.member.name} · ${isFree ? `${catIcon} ${freeTitle || "Séance libre"}` : s.session_label || "Séance"}`}
            />
            {isFree && (
              <span
                className="cst-mono"
        ...
        <div style={{ padding: "24px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          >
            ...
            <Metric
              label="RPE MOYEN"
              value={s.average_rpe != null ? Number(s.average_rpe).toFixed(1) : "—"}
            />
            {!isFree && runStats && (
              <Metric
                label="DISTANCE"
                value={runStats.distanceKm != null ? `${runStats.distanceKm} km` : "—"}
              />
            )}
            {!isFree && !runStats && (
              <Metric
                label="VOLUME TOTAL"
                value={
                  s.total_volume_kg != null ? `${Math.round(Number(s.total_volume_kg))} kg` : "—"
                }
              />
            )}
            <Metric
              label="RESSENTI"
              value={s.overall_feeling != null ? `${s.overall_feeling}/5` : "—"}
            />
          </div>
          ...
          {activePains.length > 0 && (
            ...
            >
              <div
                className="cst-mono"
                style={{ fontSize: 10, color: "#ff8a7a", letterSpacing: "0.18em" }}
              >
                🔴 DOULEUR{activePains.length > 1 ? "S" : ""} SIGNALÉE
                {activePains.length > 1 ? "S" : ""}
              </div>
              {activePains.map((p) => (
                <div key={p.id} style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <div style={{ fontSize: 13 }}>
                    <strong>{p.exercise_name}</strong> · {p.zone} · intensité {p.intensity}/5
                  </div>
                  {p.comment && (
            ...
            <div>
              ...
              <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                {(freeActivities ?? []).map((a) => {
                  const bits: string[] = [];
                  if (a.series != null || a.reps)
                    bits.push(`${a.series ?? "?"} × ${a.reps ?? "?"}`);
                  ...
                  return (
                    <div key={a.id} className="cst-card-dark" style={{ padding: 14 }}>
                      ...
                      >
                        ...
                        <h3 className="cst-display" style={{ margin: 0, fontSize: 15 }}>
                          {a.name.toUpperCase()}
                        </h3>
                      </div>
                      {bits.length > 0 && (
                        <div
                          className="cst-mono"
                          style={{ fontSize: 11, opacity: 0.75, marginTop: 6 }}
                        >
                          {bits.join(" · ")}
                        </div>
                      )}
                      {a.note && (
                        <div
                          style={{ marginTop: 6, fontSize: 12, opacity: 0.8, fontStyle: "italic" }}
                        >
                          « {a.note} »
                        </div>
                      )}
                ...
                {(!freeActivities || freeActivities.length === 0) && (
                  <div className="cst-card-dark" style={{ padding: 14, opacity: 0.6 }}>
          ...
          {(sessionMedia.length > 0 || exerciseMedia.length > 0) && (
            <div>
              <CSTSectionNum
                num={isFree ? 3 : 2}
                label="PHOTOS & VIDÉOS"
                sub={`${totalMediaCount} fichier${totalMediaCount > 1 ? "s" : ""}`}
              />
              ...
              {sessionMedia.length > 0 && (
                <div style={{ marginTop: 14 }}>
                  <div
                    className="cst-mono"
                    style={{ fontSize: 9, opacity: 0.5, letterSpacing: "0.18em", marginBottom: 8 }}
                  >
                    VIDÉOS GLOBALES SÉANCE
                  </div>
                  <div
                    style={{
                      display: "grid",
          ...
          {/* Détail par exercice (séance de programme, hors séances running sans séries) */}
          {!isFree && (byExo.size > 0 || !runStats) && (
            <div>
              ...
              <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  return (
                    <div key={name} className="cst-card-dark" style={{ padding: 16 }}>
                      ...
                      >
                        ...
                        <h3 className="cst-display" style={{ margin: 0, fontSize: 16 }}>
                          {name.toUpperCase()}
                        </h3>
                        {overTarget && (
                          <span
                            ...
                            {logs
                              ...
                              .map((l, i) => (
                                ...
                                >
                                  ...
                                  <td style={{ padding: "6px" }}>
                                    {getCoachMetricValue(planned, l.reps)}
                                  </td>
                                  <td
                                    style={{
                                      padding: "6px",
                                      color: l.rpe != null && l.rpe >= 9 ? "#E07B39" : undefined,
                                    }}
                                  >
                                    {l.rpe ?? "—"}
                                  </td>
                                  <td style={{ padding: "6px", opacity: 0.8, fontStyle: "italic" }}>
                                    {l.note || ""}
                                  </td>
                                </tr>
                              ))}
                        ...
                        >
                          {fb.rpe != null && (
                            <span style={{ marginRight: 12 }}>
                              RPE bloc : <strong>{fb.rpe}</strong>
                            </span>
                          )}
                          {fb.felt_too_hard && (
                            <span style={{ color: "#E07B39", marginRight: 12 }}>trop dur</span>
                          )}
                          {fb.felt_too_easy && (
                            <span style={{ color: "#6EAB76", marginRight: 12 }}>trop facile</span>
                          )}
                          {fb.could_not_do && (
                            <span style={{ color: "#ff8a7a", marginRight: 12 }}>
                              n'a pas pu faire
                            </span>
                          )}
                          {fb.member_comment && (
                ...
                {byExo.size === 0 && (
                  <div className="cst-card-dark" style={{ padding: 16, opacity: 0.6 }}>
          ...
          <div className="cst-card-dark" style={{ padding: 16 }}>
            <div
              className="cst-mono"
              style={{ fontSize: 9, opacity: 0.55, letterSpacing: "0.16em", marginBottom: 8 }}
            >
              💬 MOT DU COACH SUR LA SÉANCE · VISIBLE PAR LE MEMBRE
            </div>
            <textarea
              value={coachNote}
            ...
            <button
              ...
              onClick={() =>
                navigate({
                  ...
                  params: { memberId: s.member_id },
                  search: s.week_number ? { week: s.week_number + 1 } : {},
                })
              }
            >
              ADAPTER S
              {s.week_number ? `+1 (S${String(s.week_number + 1).padStart(2, "0")})` : "UIVANTE"} →
            </button>
            <button
              className="cst-btn cst-btn-ghost-dark"
              onClick={() => navigate({ to: "/coach/messages" })}
            >
              Répondre au membre
            </button>
            <button
              className="cst-btn cst-btn-ghost-dark"
              onClick={() =>
                navigate({ to: "/coach/membre/$memberId", params: { memberId: s.member_id } })
              }
            >
              Fiche membre
            </button>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="cst-card-dark" style={{ padding: 14 }}>
      <div className="cst-mono" style={{ fontSize: 9, opacity: 0.55, letterSpacing: "0.18em" }}>
        {label}
      </div>
      <div className="cst-display" style={{ fontSize: 22, marginTop: 4 }}>
        {value}
      </div>
    </div>
  );
...
function MediaThumb({
  ...
  onOpen: (media: { type: "photo" | "video"; url: string | null; caption: string | null }) => void;
}) {
  return (
    <button
      type="button"
      onClick={() => onOpen({ type: m.type, url: m.url, caption: m.caption })}
      style={{
        display: "block",
        width: "100%",
        aspectRatio: "1",
...
function MediaLightbox({
  ...
  media: { type: "photo" | "video"; url: string | null; caption: string | null } | null;
  onClose: () => void;
}) {
  useEffect(() => {
    if (!media) return;
