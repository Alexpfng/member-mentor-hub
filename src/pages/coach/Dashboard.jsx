  ...
  return (
    ...
    >
      <form
        ...
        style={{ width: 460, padding: 28, borderRadius: 14 }}
      >
        ...
        <div
          ...
          style={{ fontSize: 14, color: "var(--cst-mid-green)", marginBottom: 18 }}
        >
          un nouvel adhérent.
        </div>
        <div className="cst-col" style={{ gap: 12 }}>
          ...
          {link && (
            <div className="cst-col" style={{ gap: 6 }}>
              ...
              <span className="cst-mono" style={{ fontSize: 10, opacity: 0.6 }}>
                Valable 14 jours, utilisable une seule fois.
              </span>
            </div>
          )}
          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            <button
              type="button"
              onClick={() => {
                if (link) onDone(email || "adhérent", link);
                else onClose();
              }}
...
function CoachDashboardInner() {
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "exercise_feedbacks" }, () =>
        qc.invalidateQueries({ queryKey: ["coach"] }),
      )
  ...
  const kpis = [
    [
      String(metrics?.activeMembers ?? realMembers.length).padStart(2, "0"),
      "COACHÉS ACTIFS",
      "/coach/membres",
    ],
    [
      String(metrics?.sessionsThisWeek ?? 0).padStart(2, "0"),
    ...
    <div className="cst-screen" style={{ flexDirection: "row" }}>
      ...
      {showInvite && (
        <InviteModal
          ...
          onDone={(e) => {
            setShowInvite(false);
            setInviteOk(`Lien d'invitation généré pour ${e} — copié dans le presse-papiers.`);
      ...
      <div className="cst-col cst-scroll" style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <button
              ...
              onClick={() => navigate({ to: "/coach/import" })}
            >
              IMPORTER EXCEL ▲
            </button>
            <button
        ...
        >
          {kpis.map(([n, l, anchor], i) => (
            <div
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") navigate({ to: anchor });
              }}
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                <span
                  style={{
                    ...
                    fontSize: 56,
                    lineHeight: 0.9,
                    color: i === 3 && todayCompleted > 0 ? "#6EAB76" : "#fff",
                  }}
                >
                  {n}
                </span>
              </div>
              <span className="cst-mono" style={{ fontSize: 9, letterSpacing: "0.22em" }}>
                {l} →
              </span>
            </div>
          ))}
        ...
        <div style={{ padding: "0 32px 24px", display: "flex", gap: 12, flexWrap: "wrap" }}>
          <button
            className="cst-btn cst-btn-ghost-dark"
            onClick={() => navigate({ to: "/coach/membres" })}
          >
            MES COACHÉS →
          </button>
          <button
            className="cst-btn cst-btn-ghost-dark"
            onClick={() => navigate({ to: "/coach/seances" })}
          >
            SÉANCES →
          </button>
          {todayPlanned > 0 && (
            <button
              ...
              onClick={() => navigate({ to: "/coach/membres" })}
            >
              {todayPlanned} SÉANCE{todayPlanned > 1 ? "S" : ""} PLANIFIÉE
              {todayPlanned > 1 ? "S" : ""} CETTE SEMAINE →
            </button>
          )}
          {todayCompleted > 0 && (
            <button
              ...
              onClick={() => navigate({ to: "/coach/seances" })}
            >
              {todayCompleted} SÉANCE{todayCompleted > 1 ? "S" : ""} TERMINÉE
        ...
        {programs.length === 0 && (
          <div style={{ padding: "0 32px 32px" }}>
            <div className="cst-card-dark cst-hatch" style={{ padding: 24, textAlign: "center" }}>
              <div className="cst-display" style={{ fontSize: 22, marginBottom: 6 }}>
                AUCUN PROGRAMME ENCORE
              </div>
        ...
        {/* Bottom row: objectifs + légende côte à côte */}
        <div
          style={{
            display: "grid",
        ...
        >
          ...
          {/* Légende intensités — panneau compact vertical */}
          <div
            style={{
              borderRadius: 10,
          ...
          >
            ...
            {[
              ...
              >
                ...
                />
                <span style={{ fontSize: 11, color: "var(--cst-text-soft)" }}>{label}</span>
              </div>
            ))}
          </div>
        </div>
