    ...
    (async () => {
      try {
        ...
        const { data: sess } = await supabase
          .from("sessions")
          .select(
            "id, date, session_label, duration_minutes, average_rpe, total_volume_kg, week_number, day_number, member_note, coach_note, ended_at, session_type, free_title",
          )
          .eq("member_id", uid)
          .eq("status", "completed")
        ...
        if (ids.length) {
          const [{ data: sets }, { data: prs }, { data: stravaActivities }] = await Promise.all([
          ...
          setSessions(attachStravaActivityCardsToSessions(list, stravaActivities ?? []));
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", position: "relative" }}>
        <div className="cst-screen" style={{ minHeight: "100vh" }}>
          ...
          >
            ...
            <span className="cst-mono" style={{ color: "#fff" }}>
              HISTORIQUE
            </span>
            <span style={{ width: 18 }} />
          </div>
          ...
          <div className="cst-scroll" style={{ flex: 1, padding: "0 22px 90px" }}>
            ...
            <div
              style={{ display: "flex", gap: 6, marginTop: 16, marginBottom: 18, flexWrap: "wrap" }}
            >
              {FILTERS.map((f) => {
              ...
              groups.map((mo) => (
                <div key={mo.key} style={{ marginBottom: 24 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                    <span className="cst-display" style={{ fontSize: 14, letterSpacing: "0.1em" }}>
                      {mo.label}
                    </span>
                    ...
                    {mo.entries.map((s) => {
                      ...
                      return (
                        <div key={s.id} style={{ display: "flex", gap: 12 }}>
                          ...
                          >
                            <span className="cst-display" style={{ fontSize: 18 }}>
                              {dayNum}
                            </span>
                            <span className="cst-mono" style={{ fontSize: 8 }}>
                              {dayLabel}
                            </span>
                          </div>
                          <div
                            className="cst-card-dark cst-hatch"
                            style={{ flex: 1, padding: 14, minWidth: 0 }}
                          >
                            <div
                              style={{
                                display: "flex",
                              ...
                              const detail =
                                pr.weight_kg != null
                                  ...
                                  : pr.reps != null
                                    ? `${pr.reps} REPS`
                                    : null;
                              const label = [pr.exercise_name?.toUpperCase(), detail]
                                .filter(Boolean)
                                .join(" ");
                            ...
                            {s.strava && (
                              <div style={{ marginTop: 10 }}>
                                <StravaRunCard activity={s.strava} compact />
                              </div>
                            )}
                            ...
                            {s.coach_note && (
                              ...
                              >
                                <span
                                  className="cst-mono"
                                  style={{ fontSize: 8, color: "var(--cst-mid-green)" }}
                                >
                                  NOTE COACH
                                </span>
