...
import {
  ...
  listMemberRunningRoutes,
  deleteMemberRunningRoute,
...
interface PtEle extends Pt {
  ele: number;
}
interface EleProfile {
  km: number;
  ele: number;
}
...
const DEMO_ROUTES: RunRoute[] = [
  {
    ...
    points: [
      ...
      { lat: 46.131, lng: 3.419, ele: 261 },
      { lat: 46.136, lng: 3.415, ele: 259 },
      { lat: 46.142, lng: 3.418, ele: 258 },
      { lat: 46.148, lng: 3.425, ele: 260 },
      { lat: 46.151, lng: 3.435, ele: 265 },
      { lat: 46.149, lng: 3.448, ele: 270 },
      { lat: 46.144, lng: 3.458, ele: 275 },
      { lat: 46.137, lng: 3.462, ele: 272 },
      { lat: 46.129, lng: 3.458, ele: 268 },
      { lat: 46.124, lng: 3.449, ele: 265 },
      { lat: 46.123, lng: 3.438, ele: 263 },
      { lat: 46.125, lng: 3.429, ele: 263 },
      { lat: 46.1279, lng: 3.4245, ele: 263 },
    ],
  },
  {
    ...
    subtitle: "Vichy → Ébreuil · Technique",
    distance: 18.2,
    dplus: 480,
    dminus: 480,
    ...
    points: [
      { lat: 46.1279, lng: 3.4245, ele: 263 },
      { lat: 46.118, lng: 3.412, ele: 270 },
      { lat: 46.108, lng: 3.398, ele: 295 },
      { lat: 46.095, lng: 3.385, ele: 340 },
      { lat: 46.082, lng: 3.372, ele: 410 },
      { lat: 46.072, lng: 3.36, ele: 480 },
      { lat: 46.065, lng: 3.352, ele: 520 },
      { lat: 46.06, lng: 3.348, ele: 550 },
      { lat: 46.058, lng: 3.355, ele: 530 },
      { lat: 46.062, lng: 3.368, ele: 490 },
      { lat: 46.07, lng: 3.381, ele: 440 },
      { lat: 46.082, lng: 3.396, ele: 380 },
      { lat: 46.096, lng: 3.408, ele: 320 },
      { lat: 46.11, lng: 3.416, ele: 290 },
      { lat: 46.121, lng: 3.42, ele: 272 },
      { lat: 46.1279, lng: 3.4245, ele: 263 },
    ],
  },
  {
    ...
    points: [
      { lat: 46.1279, lng: 3.4245, ele: 263 },
      { lat: 46.115, lng: 3.44, ele: 280 },
      { lat: 46.105, lng: 3.46, ele: 320 },
      { lat: 46.092, lng: 3.482, ele: 400 },
      { lat: 46.078, lng: 3.502, ele: 510 },
      { lat: 46.065, lng: 3.52, ele: 640 },
      { lat: 46.052, lng: 3.538, ele: 780 },
      { lat: 46.042, lng: 3.552, ele: 880 },
      { lat: 46.035, lng: 3.562, ele: 950 },
      { lat: 46.03, lng: 3.57, ele: 980 },
      { lat: 46.032, lng: 3.582, ele: 960 },
      { lat: 46.04, lng: 3.595, ele: 900 },
      { lat: 46.052, lng: 3.605, ele: 820 },
      { lat: 46.068, lng: 3.61, ele: 720 },
      { lat: 46.085, lng: 3.598, ele: 600 },
      { lat: 46.102, lng: 3.58, ele: 480 },
      { lat: 46.118, lng: 3.558, ele: 360 },
      { lat: 46.1279, lng: 3.4245, ele: 263 },
    ],
  },
];
...
function downloadGPXBlob(name: string, points: PtEle[]) {
  ...
  a.href = URL.createObjectURL(
    new Blob([generateGPX(name, points)], { type: "application/gpx+xml" }),
  );
...
async function fetchOSRMSegment(a: Pt, b: Pt): Promise<{ points: Pt[]; distanceKm: number }> {
  try {
    ...
    const points = (data.routes[0].geometry.coordinates as [number, number][]).map(
      ([lng, lat]) => ({ lat, lng }),
    );
    const distanceKm = Math.round((data.routes[0].distance / 1000) * 10) / 10;
    return { points, distanceKm };
  } catch {
    return { points: [a, b], distanceKm: haversineKm(a, b) };
  }
}
...
async function uploadGPX(name: string, points: PtEle[]): Promise<string | null> {
  try {
    ...
    const { error } = await supabase.storage
      .from("running-routes")
      .upload(fileName, new Blob([gpx], { type: "application/gpx+xml" }), { upsert: true });
...
function printPDF(
  ...
  if (!win) return;
  win.document
    .write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${name} — ColosmarTraining</title>
<style>
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!createMode && browseRoute) {
        ...
        const poly = L.polyline(lls, { color: browseRoute.color, weight: 4, opacity: 0.9 }).addTo(
          map,
        );
        const mStart = L.circleMarker(lls[0], {
          radius: 8,
          fillColor: "#2D5A35",
          color: "#fff",
          weight: 2,
      ...
      if (createMode) {
        routedSegments.forEach((seg) => {
          ...
          const poly = L.polyline(
            ...
            { color: "#2D5A35", weight: 4, opacity: 0.9 },
          ).addTo(map);
        ...
        if (waypoints.length >= 2 && routedSegments.length < waypoints.length - 1) {
          ...
          const pending = L.polyline(
            [
              [waypoints[n - 1].lat, waypoints[n - 1].lng],
              [waypoints[n].lat, waypoints[n].lng],
            ] as [number, number][],
            { color: "#D4A53B", weight: 2, dashArray: "6 4", opacity: 0.7 },
          ).addTo(map);
        ...
        (window as any).__removeWP = (i: number) => {
          removeRef.current(i);
        };
      }
    })();
...
export default function MembreRunningWidget() {
  ...
  const [difficulty, setDifficulty] = useState<"facile" | "intermédiaire" | "difficile" | "expert">(
    "intermédiaire",
  );
      ...
      .then(({ routes }) => {
        setDbRoutes(
          routes.map((r: any) => ({
            ...
            color: "#2D5A35",
            center: r.points?.[0]
              ? ([r.points[0].lat, r.points[0].lng] as [number, number])
              : [46.128, 3.424],
            zoom: 13,
            points: r.points ?? [],
  ...
  const isSavedDbRoute = dbRoutes.some((r) => r.id === selectedRoute.id);
  ...
  const activeStats =
    mode === "browse"
      ? {
          ...
          dplus: selectedRoute.dplus,
          dminus: selectedRoute.dminus,
        }
      : {
          distance: osrmDistance || rawStats.distance,
          dplus: rawStats.dplus,
          dminus: rawStats.dminus,
        };
  ...
  const handleWaypointRemove = useCallback((i: number) => {
    setWaypoints((prev) => {
      ...
      if (next.length >= 2) {
        setRouting(true);
        Promise.all(next.slice(0, -1).map((wp, si) => fetchOSRMSegment(wp, next[si + 1]))).then(
          (results) => {
  ...
  async function handleSaveRoute() {
    ...
    try {
      ...
      const { id } = await saveMemberRunningRoute({
        data: {
          name: routeName.trim(),
          difficulty,
  ...
  async function handleDeleteRoute() {
    ...
    try {
      if (SUPABASE_ENABLED) {
        await deleteMemberRunningRoute({ data: { id: selectedRoute.id } });
      }
      setDbRoutes((prev) => {
        const next = prev.filter((r) => r.id !== selectedRoute.id);
        const fallback = next[0] ?? DEMO_ROUTES[0];
  ...
  return (
    <div className="cst-screen" style={{ flexDirection: "column" }}>
      <div
            ...
            {(["browse", "create"] as const).map((m) => (
              <button
                ...
                style={{
                  ...
                  background: mode === m ? "var(--cst-mid-green)" : "transparent",
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 10,
                        cursor: "pointer",
                        border: `1px solid ${on ? r.color : "rgba(255,255,255,0.07)"}`,
                        background: on ? `${r.color}22` : "#1F2A22",
                        transition: "all .15s",
                      }}
                    >
                      <div
                      ...
                      <div style={{ marginTop: 8 }}>
                        <span
                          style={{
                            ...
                            background: `${DIFF_COLOR[r.difficulty]}22`,
                            padding: "2px 7px",
                            borderRadius: 99,
                      ...
                      >
                        <div
                          style={{
                            ...
                            borderRadius: "50%",
                            background:
                              i === 0 ? "#2D5A35" : i === waypoints.length - 1 ? "#8B2318" : "#333",
                ...
                {routedSegments.length > 0 && (
                  <button
                    ...
                    style={{
                      padding: "10px 0",
                      borderRadius: 8,
                      background: "rgba(212,165,59,0.15)",
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button
                onClick={() =>
                  downloadGPXBlob(
                    mode === "browse" ? selectedRoute.name : routeName || "ma-trace",
                    activePoints,
                  )
                }
                ...
                style={{
                  ...
                  textTransform: "uppercase",
                  cursor: "pointer",
                  opacity: activePoints.length < 2 ? 0.4 : 1,
                }}
              >
                ↓ GPX
              </button>
              ...
              {mode === "create" && (
                <button
                  ...
                  style={{
                    ...
                    padding: "13px 0",
                    borderRadius: 10,
                    background: savedOk ? "rgba(45,90,53,0.6)" : "var(--cst-mid-green)",
                    border: "none",
                    color: "#fff",
                    fontFamily: "var(--cst-mono)",
                    fontSize: 10,
            ...
            {gpxUrl && (
              ...
              >
                ...
                <button
                  onClick={() => navigator.clipboard.writeText(gpxUrl)}
                  style={{
                    background: "transparent",
                    border: "none",
