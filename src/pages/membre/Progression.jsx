...
export default function Progression() {
  ...
  async function saveMeasurement() {
    ...
    if (!hasAnyMeasure) {
      toast.error(t("Renseigne au moins une mesure."));
      return;
    }
    ...
    if (error) { toast.error(t("Enregistrement impossible. Réessaie.")); setSavingMeasure(false); return; }
    setForm({ waist_cm: "", hips_cm: "", chest_cm: "", arm_cm: "", thigh_cm: "", note: "" });
    try { await loadEvolution(userId); } catch { /* ignore */ }
    setSavingMeasure(false);
    toast.success(t("Mensuration enregistrée ✓"));
  }
  ...
  async function uploadPhoto(file) {
    ...
    if (upErr) { toast.error(t("Envoi de la photo impossible. Réessaie.")); setUploadingPhoto(false); return; }
    const { error: insErr } = await supabase.from("progress_photos").insert({ member_id: userId, storage_path: path, date: localDateISO() });
    if (insErr) { toast.error(t("La photo n'a pas pu être enregistrée.")); setUploadingPhoto(false); return; }
    try { await loadEvolution(userId); } catch { /* ignore */ }
    setUploadingPhoto(false);
    toast.success(t("Photo ajoutée ✓"));
  }
  ...
  async function deletePhoto(p) {
    ...
    if (delErr) { toast.error(t("Suppression impossible. Réessaie.")); return; }
    try { await loadEvolution(userId); } catch { /* ignore */ }
  ...
  const weightSeries = useMemo(
    () => weights.map((w) => ({
      label: new Date(w.date).toLocaleDateString(locale === "en" ? "en-GB" : "fr-FR", { day: "2-digit", month: "2-digit" }),
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        <div className="cst-screen" style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 22px 8px" }}>
            ...
            <span className="cst-mono" style={{ color: "#fff" }}>{t("PROGRESSION")}</span>
            <span style={{ width: 18 }} aria-hidden="true" />
          </div>

          <div className="cst-scroll" style={{ flex: 1, padding: "0 22px 90px" }}>
            <CSTSectionNum num={1} label={t("MA PROGRESSION")} sub={`${totalSessions} ${t("SÉANCES")}`} />
            <CSTDuoTitle top={t("DEVANT")} bottom={t("moi.")} size={36} />
            ...
            {loading ? (
              ...
              <>
                ...
                <div style={{ marginTop: 18 }}>
                  <CSTSectionNum num={2} label={t("STATS GLOBALES")} />
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>
                    {[
                      ...
                      <div key={k} className="cst-card-dark" style={{ padding: 12 }}>
                        <span className="cst-mono" style={{ fontSize: 9 }}>{t(k)}</span>
                ...
                {badges && badges.earnedCount > 0 && (
                  <div style={{ marginTop: 24 }}>
                    <CSTSectionNum num={3} label={t("TROPHÉES")} sub={`${badges.earnedCount} ${t("DÉCROCHÉS")}`} />
                ...
                {profile && !profile.empty && (
                  <div style={{ marginTop: 24 }}>
                    <CSTSectionNum num={4} label={t("PROFIL DE MOUVEMENT")} />
                    <div className="cst-card-dark" style={{ marginTop: 12, padding: 14 }}>
                      <MovementRadar profile={profile} title={t("Familles de mouvement")} />
                    </div>
                  </div>
                )}
                ...
                {weightSeries.length > 1 && (
                  <div style={{ marginTop: 24 }}>
                    <CSTSectionNum num={5} label={t("POIDS DU CORPS")} sub={t("8 SEMAINES")} />
                ...
                {exercises.length > 0 && (
                  <div style={{ marginTop: 24 }}>
                    <CSTSectionNum num={6} label={t("PROGRESSION EXERCICE")} />
                    <div className="cst-card-dark" style={{ marginTop: 12, padding: 14 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 10 }}>
                        <span className="cst-mono" style={{ fontSize: 9, opacity: 0.7 }}>{t("EXERCICE")}</span>
                        <select
                          value={selected}
                        ...
                        <div style={{ height: 220 }}>
                          <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={exData.series} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                              ...
                              <Bar yAxisId="left" dataKey="weight" fill="#6EAB76" name={t("Poids (kg)")} radius={[4, 4, 0, 0]} />
                              <Line yAxisId="right" type="monotone" dataKey="rpe" stroke="#E07B39" strokeWidth={2} dot={{ fill: "#E07B39", r: 3 }} name="RPE" />
                            </ComposedChart>
                          </ResponsiveContainer>
                        </div>
                      ) : (
                        <div style={{ padding: 12, opacity: 0.5, fontSize: 12 }}>{t("Pas encore de données.")}</div>
                      )}
                    </div>
                  </div>
                )}
                ...
                {prs.length > 0 && (
                  <div style={{ marginTop: 24 }}>
                    <CSTSectionNum num={7} label={t("RECORDS PERSONNELS")} sub={`${prs.length} PR`} />
                      ...
                      {prs.map((p, i) => (
                        <div key={i} className="cst-card-dark" style={{ padding: 12 }}>
                          ...
                          <span className="cst-mono" style={{ fontSize: 9, opacity: 0.5 }}>
                            {p.date ? new Date(p.date).toLocaleDateString(locale === "en" ? "en-GB" : "fr-FR") : ""}
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                <div style={{ marginTop: 24 }}>
                  <CSTSectionNum num={8} label={t("SUIVI CORPOREL")} sub={t("MENSURATIONS (CM)")} />
                  <div className="cst-card-dark" style={{ marginTop: 12, padding: 14 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8 }}>
                      {MEASURE_FIELDS.map(([key, label]) => (
                        <div key={key}>
                          <div className="cst-mono" style={{ fontSize: 9, opacity: 0.55, letterSpacing: "0.12em", marginBottom: 4 }}>{t(label).toUpperCase()}</div>
                          <input
                            value={form[key]}
                    ...
                    >
                      {savingMeasure ? t("ENREGISTREMENT…") : t("ENREGISTRER MES MENSURATIONS")}
                    </button>
                  </div>
                  {measures.length > 0 && (
                    <div className="cst-col" style={{ gap: 6, marginTop: 10 }}>
                      {measures.map((m) => {
                        const parts = MEASURE_FIELDS.filter(([k]) => m[k] != null).map(([k, l]) => `${t(l)} ${m[k]}`);
                        return (
                          <div key={m.id} className="cst-card-dark" style={{ padding: "10px 12px", display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
                            <span className="cst-mono" style={{ fontSize: 10, opacity: 0.6 }}>{new Date(m.date).toLocaleDateString(locale === "en" ? "en-GB" : "fr-FR")}</span>
                ...
                {/* Photos d'évolution */}
                <div style={{ marginTop: 24 }}>
                  <CSTSectionNum num={9} label={t("PHOTOS D'ÉVOLUTION")} sub={t("PRIVÉ · VISIBLE PAR TON COACH")} />
                  <label
                  ...
                  >
                    {uploadingPhoto ? t("ENVOI…") : t("📷 AJOUTER UNE PHOTO")}
                    <input
                      type="file"
                      accept="image/*"
                      ...
                      {photos.map((p) => (
                        <div key={p.id} style={{ position: "relative", aspectRatio: "3/4", borderRadius: 8, overflow: "hidden", background: "#111" }}>
                          ...
                          <span style={{ position: "absolute", left: 0, bottom: 0, right: 0, padding: "3px 6px", background: "rgba(0,0,0,0.6)", color: "#fff", fontSize: 9, fontFamily: "var(--cst-mono)" }}>
                            {new Date(p.date).toLocaleDateString(locale === "en" ? "en-GB" : "fr-FR")}
                          </span>
                          <button
                ...
                {prs.length === 0 && exercises.length === 0 && (
                  <div className="cst-card-dark" style={{ marginTop: 24, padding: 22, textAlign: "center" }}>
                    <div className="cst-display" style={{ fontSize: 18, marginBottom: 6 }}>{t("PAS ENCORE DE DONNÉES")}</div>
                    <p style={{ margin: 0, fontSize: 13, opacity: 0.65 }}>
                      {t("Termine ta première séance pour voir ta progression ici.")}
                    </p>
                  </div>
                )}
              </>
