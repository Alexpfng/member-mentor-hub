  ...
  return (
    <>
      {items.map(it => {
        ...
        return (
          <div key={it.id} onClick={() => { navigate({ to: it.path }); onNav?.(); }} style={{
  ...
  return (
    <>
      ...
      <div className="coach-mobile-topbar">
        <button className="coach-hamburger" onClick={() => setDrawerOpen(true)} aria-label="Ouvrir le menu">
        ...
        <button onClick={async () => { await supabase.auth.signOut(); navigate({ to: '/login' }); }}
          style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.6)', borderRadius: 6, padding: '6px 10px', fontSize: 10, cursor: 'pointer', fontFamily: 'var(--cst-mono)', marginLeft: 'auto' }}>
          ⎋
        </button>
      </div>

      {/* ── MOBILE DRAWER OVERLAY ── */}
      <div className={`coach-drawer-overlay ${drawerOpen ? 'open' : ''}`} onClick={() => setDrawerOpen(false)} />
      ...
      <div className={`coach-drawer ${drawerOpen ? 'open' : ''}`}>
        <div style={{ padding: '20px 16px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            <CSTLogo />
            <button onClick={() => setDrawerOpen(false)} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 22, cursor: 'pointer', padding: 4 }}>✕</button>
          </div>
