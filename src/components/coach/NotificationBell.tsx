...
export default function NotificationBell() {
  ...
  return (
    <div style={{ position: "relative" }}>
      <button
        onClick={() => { setOpen(false); navigate({ to: "/coach" }); }}
        title={`${count} à traiter`}
        aria-label="Notifications"
        style={{
          background: "transparent", border: "1px solid var(--cst-btn-ghost-border)",
          color: "var(--cst-text-soft)", borderRadius: 6, padding: "6px 10px",
          fontSize: 14, cursor: "pointer", position: "relative", minHeight: 36,
        }}
      >
        🔔
        {count > 0 && (
          <span style={{
            ...
            borderRadius: 999, fontSize: 9, padding: "1px 5px", fontFamily: "var(--cst-mono)",
