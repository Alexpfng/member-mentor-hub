...
import { getArchivedCoachSessions, restoreSessionToCoachDashboard } from "@/lib/coach-dashboard.functions";
  ...
  return (
    <div className="cst-card-dark" style={{ padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          <div className="cst-display" style={{ fontSize: 18, marginBottom: 4 }}>INFOS ARCHIVÉES</div>
        ...
        >
          {open ? "MASQUER" : `VOIR${archivedSessions.length ? ` (${archivedSessions.length})` : ""}`} →
        </button>
      </div>
          ...
          {isLoading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {archivedSessions.map((s) => {
                ...
                return (
                  <div key={s.id} className="cst-card-dark" style={{ padding: 14, display: "flex", gap: 14, alignItems: "flex-start" }}>
                    <CSTAvatar initials={initials} size={36} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                        <strong style={{ fontSize: 14 }}>{s.memberName.toUpperCase()}</strong>
                        <span style={{ fontSize: 12, opacity: 0.7 }}>{label}</span>
                        <span className="cst-mono" style={{ fontSize: 10, opacity: 0.55, marginLeft: "auto" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
                      ...
                      <button
                        ...
                        style={{ background: "rgba(45,90,53,0.15)", border: "1px solid rgba(45,90,53,0.4)", color: "#6EAB76" }}
